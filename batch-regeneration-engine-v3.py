#!/usr/bin/env python3
"""
Motor de Regeneración Batch v3.0 - SINTOMARIO.ORG
Regenera todos los artículos existentes aplicando la estrategia narrativa Saramago + Osho + Jung
"""
...
# Importar el motor narrativo v3.0 con capa junguiana
from narrative_engine_v3 import NarrativeEngine, METAPHOR_BANK, SARAMAGO_ANCHORS

class BatchRegenerationEngineV3:
    def __init__(self):
        self.client = OpenAI(api_key=os.getenv("OPENAI_API_KEY", "TU_API_KEY_AQUI"))
        self.narrative_engine = NarrativeEngine()
        self.backup_dir = "backup_originals_v3"
        self.generated_dir = "generated_batch_v3"
    ...
    def backup_original(self, article_info):
        ...
        try:
            ...
            backup_filename = f"{article_info['zona']}_{article_info['contexto']}_original_v3.html"
            ...
            print(f"[Backup v3.0] {backup_filename}")
            return backup_path
    ...
    def generate_new_content(self, zona, contexto):
        """Genera nuevo contenido usando el motor narrativo v3.0"""
        try:
            ...
            # Generar artículo con el motor narrativo v3.0
    ...
    def create_html_template(self, zona, contexto, content):
        ...
        # Crear metadatos SEO
        keywords = f"{zona.lower()} {contexto.lower()}, dolor de {zona.lower()}, {contexto.lower()} corporal, síntomas emocionales, lectura profunda, análisis somático"
...
<head>
  ...
  <title>{title}</title>
  <meta name="description" content="{title}: lectura corporal profunda con perspectiva junguiana, contexto emocional y práctica de integración.">
  <link rel="canonical" href="https://sintomario.org/cuerpo/{zona}/{contexto}">
  <meta property="og:title" content="{title}">
  <meta property="og:description" content="{title}: lectura corporal profunda con perspectiva junguiana, contexto emocional y práctica de integración.">
  ...
  {{
    ...
    "@type": "MedicalWebPage",
    "headline": "{title}",
    "description": "{title}: lectura corporal profunda con perspectiva junguiana, contexto emocional y práctica de integración.",
...
<body>
  ...
  <main class="site-main">
    <div class="container">
      <article class="card">
        ...
        <!-- Contenido generado narrativamente con capa junguiana -->
        <div class="narrative-content-jung">
    ...
    <div class="container">
      ...
      <div class="footer-bottom">
        ...
        <div class="disclaimer-compact">
          ...
          <span> contenido educativo profundo </span>
        </div>
      </div>
    </div>
  </footer>
  ...
  <script>
    ...
    def regenerate_article(self, article_info):
        ...
        # 2. Generar nuevo contenido narrativo con capa junguiana
        ...
        new_filename = f"{zona}_{contexto}_regenerado_v3.html"
        ...
        try:
            ...
            print(f"[Generado v3.0] {new_filename} ({len(new_html)} chars)")
            return True
    ...
    def regenerate_batch(self, limit=None):
        """Regenera todos los artículos en batch con capa junguiana"""
        ...
        if limit:
            ...
            print(f"Regenerando todos los {total_articles} artículos con capa junguiana profunda")
        
        print(f"\n{'='*60}")
        print(f"INICIO DE REGENERACIÓN BATCH v3.0 - SARAMAGO + OSHO + JUNG")
        print(f"Directorio backup: {self.backup_dir}")
        print(f"Directorio generado: {self.generated_dir}")
        print(f"Motor narrativo: v3.0 con capa junguiana profunda")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"REGENERACIÓN COMPLETADA v3.0")
        print(f"Exitosos: {successful}")
        print(f"Fallidos: {failed}")
        print(f"Total: {successful + failed}")
        print(f"Backup en: {self.backup_dir}")
        print(f"Generados en: {self.generated_dir}")
        print(f"Estilo: Saramago + Osho + Jung (capa profunda)")
        print(f"{'='*60}")
        
        return successful, failed
    
    def generate_report(self):
        """Genera un reporte de la regeneración v3.0"""
        report = {
            'timestamp': datetime.now().isoformat(),
            'backup_directory': self.backup_dir,
            'generated_directory': self.generated_dir,
            'articles_processed': len(self.discover_articles()),
            'narrative_engine_version': '3.0',
            'model_used': 'gpt-4o',
            'strategy': 'clinical-contemplative-saramago-osho-jung',
            'jungian_layer': 'deep-archetypal-without-jargon',
            'features': [
                'saramago_rhythm',
                'osho_paradox',
                'jungian_depth',
                'no_jargon_jungian',
                'mythological_body_reading',
                'clinical_accuracy'
            ]
        }
        
        report_path = os.path.join(self.generated_dir, 'regeneration_report_v3.json')
        with open(report_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        print(f"[Reporte v3.0] Generado en: {report_path}")

def main():
    engine = BatchRegenerationEngineV3()
    
    # Opciones de ejecución
    print("=== MOTOR DE REGENERACIÓN BATCH v3.0 - SINTOMARIO.ORG ===")
    print("ESTILO: Saramago + Osho + Jung (capa profunda sin jerga)")
    print("1. Regenerar 5 artículos (prueba junguiana)")
    print("2. Regenerar 20 artículos (prueba extendida)")
    print("3. Regenerar todos los artículos (producción completa)")
    ...
    try:
        ...
        if option == "1":
            ...
            confirm = input("¿Confirmar regeneración de TODOS los artículos con capa junguiana profunda? (s/N): ").strip().lower()
            if confirm == 's':
