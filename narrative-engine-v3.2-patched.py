#!/usr/bin/env python3
"""
Motor Narrativo SINTOMARIO.ORG v3.2-PATCHED
Nivel Industrial: Sistema de Ensamblaje Lingüístico Avanzado
Correcciones críticas para producción masiva sin fallos
"""
...
# System Prompt Principal LIMPIO (sin variables dinámicas)
SYSTEM_PROMPT = """
...
class NarrativeEngineV3_2_PATCHED:
    def __init__(self):
        self.model = "gpt-4o" 
...
"""
    ...
    def generate_article(self, organ, emotion):
        print(f"\n[Motor v3.2-PATCHED] Generando: {organ} + {emotion}")
        
        metaphor = self.get_random_metaphor(organ)
        print(f"[Motor v3.2-PATCHED] Metáfora de anclaje: {metaphor}")
        
        max_attempts = 3
        for attempt in range(max_attempts):
            print(f"[Motor v3.2-PATCHED] Intento {attempt + 1}/{max_attempts}")
            
            text_base, error1 = self.generate_phase1(organ, emotion, metaphor)
            if error1:
                print(f"[Motor v3.2-PATCHED] Error Fase 1: {error1}")
                continue
                
            text_expanded, error2 = self.generate_phase2(organ, emotion, metaphor, text_base)
            if error2:
                print(f"[Motor v3.2-PATCHED] Error Fase 2: {error2}")
                continue
            ...
            if not approved:
                print(f"[Motor v3.2-PATCHED] Rechazado por Quality Gate: {reason}")
                if attempt < max_attempts - 1:
                    print(f"[Motor v3.2-PATCHED] Regenerando...")
                    continue
                else:
                    print(f"[Motor v3.2-PATCHED] Usando última versión despite Quality Gate")
...
metaphor: "{metaphor}"
version: "3.2-industrial-patched"
...
\n"""
        ...
        print(f"[Motor v3.2-PATCHED] Falló la generación después de {max_attempts} intentos")
        return ""
    ...
    def save_article(self, organ, emotion, content):
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"[Motor v3.2-PATCHED] Guardado en: {filepath}")
            return filepath
        except Exception as e:
            print(f"Error E/S: {e}")
            return None
...
def main():
    engine = NarrativeEngineV3_2_PATCHED()
    ...
    if not os.path.exists('metaphors.json'):
        ...
        print("[Motor v3.2-PATCHED] Creado metaphors.json")
    ...
    if not os.path.exists('few_shot_examples.json'):
        ...
        print("[Motor v3.2-PATCHED] Creado few_shot_examples.json")
    ...
    if not os.path.exists('resonance_matrix.json'):
        ...
        print("[Motor v3.2-PATCHED] Creado resonance_matrix.json")
    
    # Artículos de prueba con sistema industrial parchado
    ...
    for organ, emotion in articles_to_generate:
        ...
        if article:
            ...
            print(f"[Motor v3.2-PATCHED] Completado ({len(article)} chars)")
        else:
