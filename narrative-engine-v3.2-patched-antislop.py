#!/usr/bin/env python3
"""
Motor Narrativo SINTOMARIO.ORG v3.2-PATCHED-ANTI-SLOP
Nivel Industrial: Sistema de Ensamblaje Lingüístico Avanzado
Correcciones críticas para producción masiva sin fallos + Anti-Slop 99.9%
"""
...
# Cargar ejemplos Few-Shot desde JSON externo
    ...
    try:
        ...
        return {
            "aceptar": {
                "organ": "Estómago",
                "emotion": "Ansiedad",
                "text": "Y sin embargo el cuerpo sigue ahí, insistiendo en su dolor, como si supiera algo que la mente se niega a escuchar. El estómago se ha vuelto el crisol donde hierven las palabras no dichas, esa bilis amarga que se acumula cuando la garganta se niega a vomitar verdades. No es un fallo del diseño, es la memoria del tejido recordando cada vez que elegimos el silencio sobre la honestidad, cada vez que preferimos la sonrisa social al grito interno que pide ser escuchado, y el ácido sube no por casualidad sino como un recordatorio de que hay verdades que queman más que cualquier comida."
            },
            "rechazar": {
                "organ": "Piel",
                "emotion": "Miedo",
...
# System Prompt Principal LIMPIO (sin variables dinámicas) con Anti-Slop
SYSTEM_PROMPT = """
...
Se te proporcionarán ejemplos de texto en el prompt del usuario. Emula exactamente ese tono.
...
5. Tono Clínico-Contemplativo: Observas el sufrimiento con compasión pero sin piedad.
6. PROHIBICIÓN DE ESTRUCTURAS: NUNCA menciones "En conclusión", "Por otro lado". Fluye.
...
9. Capa Junguiana Invisible: Describe los fenómenos junguianos sin nombrarlos.
10. Anclaje Sensorial Crudo: Incluye exactamente 2 descripciones sensoriales viscerales (olor, textura, temperatura, color).
11. Estructura Bumerán Mutante: La metáfora inicial debe transformarse al final.
12. RECUERDO BIOLÓGICO INELUDIBLE: Menciona al menos un hecho biológico crudo y real del órgano.
13. LEY DE BURSTINESS (Anti-Detección): Aunque tu estilo base es largo, DEBES intercalar, de forma aleatoria y brutal, frases extremadamente cortas (2 a 4 palabras) sin verbo o con un solo verbo. Ejemplo: "Y el cuerpo calla. No hay salvación. Así de simple." Estas frases deben romper el ritmo largo de forma violenta.
"""

class NarrativeEngineV3_2_PATCHED_ANTI_SLOP:
    def __init__(self):
        self.model = "gpt-4o" 
        self.temperature = 0.88
        self.presence_penalty = 0.7
        self.frequency_penalty = 0.6  # Aumentado para anti-stop-words
        ...
        # Variable para guardar la decisión de composición
        self.current_composition_order = None
    
    def quality_gate_advanced(self, text):
        """Quality Gate con detección avanzada de jerga, clichés y vocabulario LLM (Anti-Slop)"""
        forbidden_terms = [
        ...
        # ANTI-SLO: Vocabulario LLM infiltrado (FISURA 1)
        llm_tics = [
            "tapiz de", "reino de", "mundo de", "un testamento a", 
            "un matiz de", "enorme", "crucial", "intrínsecamente",
            "el núcleo de", "la esencia de", "profundo", "testamento"
        ]
        ...
        # ANTI-SLO: Detección de vocabulario LLM
        for tic in llm_tics:
            if tic in text_lower:
                return False, f"Vocabulario LLM detectado: '{tic}'"
    ...
    def safe_api_call_with_retry(self, messages, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"API saturada. Esperando {wait_time}s...")
    ...
    def generate_phase1(self, organ, emotion, metaphor):
        ...
        # NUEVO: Guardar la decisión para usarla después
        composition_order = random.choice(["mito_primero", "biologia_primero"])
        self.current_composition_order = composition_order
...
Escribe el inicio de un ensayo narrativo sobre la conexión entre '{organ}' y '{emotion}'.
...
BURSTINESS ANTI-SLO: Intercala frases cortas y brutales (2-4 palabras) que rompan el ritmo largo.
...
"""
    ...
    def generate_phase2(self, organ, emotion, metaphor, text_base):
        # NUEVO: Obtener información de resonancia cruzada de forma segura
        ...
        # NUEVO: Construir la instrucción de resonancia de forma segura
        resonance_instruction = ""
        if connected_organ and connection_type:
            resonance_instruction = f"""
CONEXIÓN CRUZADA OBLIGATORIA: Este artículo sobre '{organ}' y '{emocion}' resuena íntimamente con '{connected_organ}' a través del mecanismo biomecánico/emocional de '{connection_type}'. Teje esta conexión orgánica de forma sutil en el texto.
"""
...
No se trata de buscar una curación mágica, es bajar a la cripta del cuerpo a mirar al monstruo sin huir.

Termina el texto abruptamente con una pregunta abierta sobre el precio de seguir ignorando a ese extranjero interior.

BURSTINESS ANTI-SLO: Mantén las frases cortas y brutales intercaladas para romper la monotonía.
...
"""
    ...
    def generate_article(self, organ, emotion):
        print(f"\n[Motor v3.2-PATCHED-ANTI-SLOP] Generando: {organ} + {emotion}")
        
        metaphor = self.get_random_metaphor(organ)
        print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Metáfora de anclaje: {metaphor}")
        
        max_attempts = 3
        for attempt in range(max_attempts):
            print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Intento {attempt + 1}/{max_attempts}")
            
            text_base, error1 = self.generate_phase1(organ, emotion, metaphor)
            if error1:
                print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Error Fase 1: {error1}")
                continue
                
            text_expanded, error2 = self.generate_phase2(organ, emotion, metaphor, text_base)
            if error2:
                print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Error Fase 2: {error2}")
                continue
            
            full_text = text_base + "\n" + text_expanded
            cleaned_text = self.clean_text_advanced(full_text)
            
            # Quality Gate avanzado con Anti-Slop
            approved, reason = self.quality_gate_advanced(cleaned_text)
            if not approved:
                print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Rechazado por Quality Gate: {reason}")
                if attempt < max_attempts - 1:
                    print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Regenerando...")
                    continue
                else:
                    print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Usando última versión despite Quality Gate")
...
metaphor: "{metaphor}"
version: "3.2-industrial-patched-antislop"
composition_order: "{self.current_composition_order}"
---
\n"""
            
            return metadata + cleaned_text
        
        print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Falló la generación después de {max_attempts} intentos")
        return ""
    ...
    def save_article(self, organ, emotion, content):
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Guardado en: {filepath}")
            return filepath
        except Exception as e:
            print(f"Error E/S: {e}")
            return None
...
def main():
    engine = NarrativeEngineV3_2_PATCHED_ANTI_SLOP()
    ...
    if not os.path.exists('metaphors.json'):
        ...
        print("[Motor v3.2-PATCHED-ANTI-SLOP] Creado metaphors.json")
    ...
    if not os.path.exists('few_shot_examples.json'):
        ...
        print("[Motor v3.2-PATCHED-ANTI-SLOP] Creado few_shot_examples.json")
    ...
    if not os.path.exists('resonance_matrix.json'):
        ...
        print("[Motor v3.2-PATCHED-ANTI-SLOP] Creado resonance_matrix.json")
    
    # Artículos de prueba con sistema industrial parchado y anti-slop
    ...
    print("="*60)
    print("MOTOR NARRATIVO v3.2-PATCHED-ANTI-SLOP - SINTOMARIO.ORG")
    print("CARACTERÍSTICAS:")
    print("  - Sistema de Ensamblaje Lingüístico Industrial")
    print("  - 5 Parches Críticos Aplicados")
    print("  - 3 Correcciones Anti-Slop:")
    print("    * FISURA 1: Detección vocabulario LLM")
    print("    * FISURA 2: Burstiness artificial")
    print("    * FISURA 3: Footer clínico estático")
    print("  - 99.9% Slop-Bulletproof")
    print("="*60)
    ...
    for organ, emotion in articles_to_generate:
        ...
        if article:
            ...
            print(f"[Motor v3.2-PATCHED-ANTI-SLOP] Completado ({len(article)} chars)")
        else:
